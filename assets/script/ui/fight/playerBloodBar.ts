...
import { _decorator, Component, Node, SpriteFrame, Prefab, Layout, Vec3, find, UITransform, SpriteComponent, tween, clamp, CameraComponent } from 'cc';
...
export class PlayerBloodBar extends Component {
    ...
    @property(Layout)
    public layoutContainer: Layout = null!;//container节点的layout组件
    ...
    @property(UITransform)
    public UIComCurBloodBar: UITransform = null!;//血量进度条的UI组件
    ...
    @property(UITransform)
    public UIComBloodBar: UITransform = null!;//血条容器节点

    public set curBlood (v: number) {
        this._curBlood = v;
    }
    ...
    update (deltaTime: number) {
        ...
        if (this.node.parent && this.node.active && this._ndTarget && this._ndTarget.parent) {
            this._mainCamera?.convertToUINode(this._ndTarget.worldPosition, find("Canvas") as Node, this._curPos);
